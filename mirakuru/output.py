...
to be matched. See :meth:`OutputExecutor._consume_until_banner_or_block`.
"""
...
class OutputExecutor(SimpleExecutor):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # has already been consumed. See `_consume_until_banner_or_block`.
    ...
    def _consume_until_banner_or_block(self, output: IO[Any]) -> tuple[bool, bool]:
        ...
        Must only be called for a descriptor that has been reported readable,
        as peeking into an empty buffer would block.

        Returns a pair (found, should_break):
        - found: banner was detected and consumed up to end-of-line.
        - should_break: nothing has been consumed, because no more data is
          immediately available for this descriptor, so the caller's inner
          draining loop should break.
        """
    ...
    def _wait_for_darwin_output(self, *fds: IO[Any] | None) -> bool:
        ...
        Drain available data in chunks from ready descriptors and look for the
        banner using regex.search on a rolling buffer. This avoids blocking on
        TextIOWrapper.readline() with partial data and prevents pipe
        backpressure under heavy pre-banner output.
        """
    ...
    def _wait_for_output(self, *polls: tuple["select.poll", IO[Any]]) -> bool:
        ...
        chunked reads to avoid stalling on text line buffering. Returns True as
        soon as the banner is detected using regex.search().
